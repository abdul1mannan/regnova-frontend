...
export default function ChatWindow({ messages, isLoading = false }: ChatWindowProps) {
  ...
  // When a new message appears, remove all "sending" status from previous messages
  useEffect(() => {
    if (messages.length > 0 && messages[messages.length - 1].status === "sent") {
      // Find and handle any "sending" status messages
      const hasSendingMessages = messages.some(msg => msg.status === "sending" && msg.id !== messages[messages.length - 1].id);
      
      if (hasSendingMessages) {
        // This would be handled by the parent component to update message states
        console.log("New complete message arrived - sending statuses should be cleared");
      }
    }
  }, [messages]);
  ...
  return (
    <div className="space-y-6 pb-6 max-w-4xl mx-auto">
      {messages.slice(0, visibleMessages).map((msg, index) => {
        // Skip showing status indicators if a new message has arrived after this message
        const isOldSendingMessage = 
          msg.status === "sending" && 
          index < messages.length - 1 && 
          messages[index + 1].sender === "REGNOVA Bot" &&
          messages[index + 1].status === "sent";
            ...
            >
              ...
              <div className="space-y-1 max-w-[calc(100%-3rem)]">
                ...
                <div className="flex items-center px-2">
                  ...
                  {msg.status === "sending" && !isOldSendingMessage && (
                    <span className="ml-2 flex space-x-0.5">
                      <span className="h-1.5 w-1.5 rounded-full bg-blue-400 animate-pulse" style={{ animationDelay: "0ms" }}></span>
                      <span className="h-1.5 w-1.5 rounded-full bg-blue-400 animate-pulse" style={{ animationDelay: "150ms" }}></span>
                      <span className="h-1.5 w-1.5 rounded-full bg-blue-400 animate-pulse" style={{ animationDelay: "300ms" }}></span>
                    </span>
                  )}
      ...
      {isLoading && (
        <div className="flex justify-start mt-4 opacity-0 animate-fade-in message-container" style={{ animationFillMode: "forwards" }}>
